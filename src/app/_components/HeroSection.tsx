...
import { useGetAllFlightsQuery } from '@/hooks/api/useGetFlightsQuery';
import { FlightResults } from './FlightResults';
import { mapFlights } from '@/utils/formater';
...
export function HeroSection() {
  ...
  const allFlights = data?.pages.flatMap((page) => page.data ?? []) || [];
  const dictionaries = data?.pages?.[0]?.dictionaries ?? {};

  const flightResults: Flight[] = mapFlights(allFlights, dictionaries);

  return (
    <>
      <div className="relative min-h-[849px] lg:min-h-[600px] lg:overflow-hidden">
        <BackgroundSlider
          autoplayDelay={2000}
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4 z-20">
          <div className="mb-8">
            ...
            <p className="text-base sm:text-lg md:text-xl lg:text-2xl text-white/90 max-w-3xl animate-fade-in-up animation-delay-300 px-2">
              Discover the best flights at unbeatable prices. Search, compare,
              and book your perfect journey.
            </p>
          </div>
      ...
      <FlightResults flights={flightResults} isSearching={isLoading} />
    </>
  );
}
